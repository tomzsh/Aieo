...
> Masih ada **bug**, fitur belum stabil, dan **perubahan bisa merusak**.  
...
| Job optimasi panjang | Butuh app/job worker tetap hidup sampai selesai |

---

## Apa itu Aieo?

Aieo adalah **tool internal redaksi**:

```
Draf mentah  →  Optimasi / parafrase (LLM)  →  Review + SEO  →  WordPress
 (tempel / URL / RSS)      (job async)         (edit, image)    (draft / publish / schedule)
```

**Bukan** CMS pengganti WordPress. Aieo menyiapkan naskah, lalu mengirim ke WP lewat REST API.
...
| **Prompt template** | Versi template per workspace (seed v2/v3) |
| **Validasi pasca-LLM** | Post-process agar tidak mengarang fakta liar |
...
| Schedule | Post `future` + tanggal/jam |
| Kategori & tag | Sync kategori; auto create tag |
| Meta SEO | Yoast + Rank Math (jika aktif di REST) |
...
| Fitur | Keterangan |
|-------|------------|
| Antrian job | queued / running / completed / failed |
...
| Tema gelap/terang | `html.light` / `html.dark` |
| Bahasa UI | Toggle **ID \| EN** (cookie + localStorage) |
| Enkripsi rahasia | Password WP & API key at-rest |
...
Browser (Next.js App Router)
    ...
    └─ Publish / media ───────► WordPress REST (/wp-json atau ?rest_route=)
```
...
| LLM | SDK OpenAI-compatible (`openai`) |
| WordPress | REST API + Application Passwords |
| Secrets | `AIEO_ENCRYPTION_KEY` (enkripsi at-rest di app) |
...
| `SUPABASE_SERVICE_ROLE_KEY` | Service role (**hanya server**) |
...
Provider/model **per workspace** juga bisa diisi di UI: **Settings → LLM**.
...
5. Pilih **model** (list dari `GET /models` jika didukung)  
6. **Tes koneksi**  
7. Opsional: fallback provider/model  

### Featured image (generate)

Di halaman LLM yang sama, atur **model image** terpisah (model chat ≠ model gambar):
...
1. Role user **Editor** atau **Administrator**  
...
4. **Tes** → hijau **TERHUBUNG · SIAP PUBLISH**  
...
Prioritas saat publish (jika featured image aktif):
...
Dokumentasi lebih panjang di app: **`/docs`**.
...
└── src/
    ...
    ├── app/                  # UI + API routes
    ├── components/
    └── lib/                  # i18n, llm, jobs, wordpress, supabase, …
```
...
| Gejala | Cek |
|--------|-----|
| Halaman **blank putih** lama | Compile dev pertama 10–40 dtk; hard refresh (`Ctrl+Shift+R`). Middleware tidak boleh hang di Supabase (sudah ada timeout). |
| Signup/login gagal | Supabase Auth Email aktif; cek URL + anon key |
| Optimasi gagal | Settings LLM: key, base URL, model; cek tab Jobs |
...
| Publish “tidak muncul” | Mode **Draft** → Posts → Drafts, bukan homepage |
...
**Aieo** · rilis awal · [github.com/tomzsh/Aieo](https://github.com/tomzsh/Aieo)  
**Dokumentasi English (utama):** [README.md](./README.md)
